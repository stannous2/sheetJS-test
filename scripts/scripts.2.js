   ...
   reader.onload = function (e) {
    ...
    for (let R = cell_range.s.r; R <= cell_range.e.r; ++R) {
     for (let C = cell_range.s.c; C <= cell_range.e.c; ++C) {
      ...
      if (cell && colHeaderArray.includes(cell.v)) {
...
function createTable(columnHeaderArr) {
 let headerCell = ""
 let headerRow
 for (let i = 1; i < columnHeaderArr.length; i++) {
  headerCell += "<td>" + columnHeaderArr[i] + "</td>"
 };
 ...
 inputColumnHeaderFile.change(function (e) {
  ...
  if (!f) {
   ...
   r.onload = function (e) {
    ...
    contents = contents.toString().replace(/\s+/g, '').trim()

    contents = contents.split(', ')
    for (i = 0; i < contents.length; i++) {
     colHeaderArray.push(contents[i])
     console.log('colHeaderArray ', colHeaderArray)
    }
    // colHeaderArray.forEach(function (item) {
    //  console.log('colHeaderArray ', item)
    // })
